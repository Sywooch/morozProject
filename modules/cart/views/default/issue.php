...
<?if(count($goods)>0):?>
    ...
    <?if(!\Yii::$app->user->isGuest):?>
        ...
        <div class="row">
            <div class="col-md-6">
                <h3>Контактная информация</h3>
                    ...
                    <?= $form->field($model, 'comment')->textarea()->label("Коментарий"); ?>
                    <?if(\Yii::$app->user->isGuest):?>
        ...
        <div class="line-price">
            ...
            <?= Html::submitButton('ЗАКАЗАТЬ', ['class' => 'btn btn-primary', 'name' => 'signup-button']) ?>
        </div>
        <?php ActiveForm::end(); ?>
    <?else:?>
        <div class="tabs-login-sign">
            <!-- Nav tabs -->
            <ul class="nav nav-tabs" role="tablist">
                <li role="presentation" class="first-tb active"><a href="#login" aria-controls="login" role="tab" data-toggle="tab">Вход</a></li>
                <li role="presentation"><a href="#sign" aria-controls="sign" role="tab" data-toggle="tab">Регистрация</a></li>
            </ul>
...
<? $this->registerJs("
    $(function(){
        ...
        input.click(function(){
            $('#district').remove();
            if($(this).val()==='nov' && $(this)[0].checked){
                $(this).next().next().after(\"<select name='district' id='district'><option value='300'>Дзержинский район</option><option value='200'>Железнодорожный район</option><option value='200'>Заельцовский район</option><option value='400'>Калининский район</option><option value='600'>Кировский район</option><option value='200'>Заельцовский район</option><option value='700'>Ленинский район</option><option value='200'>Октябрьский район</option><option value='1000'>Первомайский район</option><option value='200'>Советский район</option><option value='200'>Центральный район</option></select>\")
                var pr = $('#district').attr('selected', 'selected').val();
                $('#dprice').html(pr+\" <span class='glyphicon glyphicon-ruble'></span> \");
            }
